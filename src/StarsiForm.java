...
public class StarsiForm {
    ...
    public StarsiForm(int starsId) {
        this.starsId = starsId;

        window = new JFrame("Starši Obrazec"); // Ustvarimo novo okno
        ...
        mainTitle = new JLabel("Starši Obrazec"); // Ustvarimo nov label
        ...
        imeLabel = new JLabel("Ime starša:"); // Ustvarimo nov label
        ...
        priimekLabel = new JLabel("Priimek starša:"); // Ustvarimo nov label
        ...
        emailLabel = new JLabel("Email starša:"); // Ustvarimo nov label
        ...
        gesloLabel = new JLabel("Geslo starša:"); // Ustvarimo nov label
        ...
        if (starsId != 0) {
            try {
                ...
                String query = "SELECT * FROM \"Starsi\" WHERE \"id\" = " + starsId + ";";
...
//                CREATE OR REPLACE FUNCTION dodaj_starsa(p_ime character varying, p_priimek character varying, p_email character varying, p_geslo character varying)
                String query = "SELECT dodaj_starsa('" + ime + "', '" + priimek + "', '" + email + "', '" + geslo + "');";
                db.executeQuery(query);
            } else {
//                CREATE OR REPLACE FUNCTION posodobi_starsa(p_id integer, p_ime character varying, p_priimek character varying, p_email character varying, p_geslo character varying)
                String query = "SELECT posodobi_starsa(" + starsId + ", '" + ime + "', '" + priimek + "', '" + email + "', '" + geslo + "');";
                db.executeQuery(query);
            }
            window.dispose();
        } catch (SQLException e) {
